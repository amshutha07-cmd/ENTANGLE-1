...
With --tunnel the address is also saved as this computer's relay setting, so the app on THIS computer is
already connected. Everyone else pastes the printed address into Settings → Connection.

Keep this window open while people use the relay. Ctrl+C stops everything. The free tunnel address changes
every time you start it; the relay's data (names, waiting files) lives in ./relay_data and survives restarts.
"""
...
def announce(url: str, save: bool) -> None:
    print(f"\n[tunnel] address created: {url}\n[tunnel] checking that it works from the internet…")
...
def main() -> int:
    ...
    ap.add_argument("--tunnel", action="store_true", help="also create a free public https address (Cloudflare)")
    ...
    if relay is None and tunnel is None:
        print("Nothing to start: a relay is already running and no tunnel was requested (add --tunnel).")
        return 0
